...
// Version: 1.0.1
// Author: Nova & hjhjd
// Fix: success() 输出包裹 result 字段，对齐 Plugin.js processToolCall 期望
// ============================================================
...
function dispatchCommand(command, args, repos, envConfig) {
  ...
  process.stdin.on('end', () => {
    try {
      ...
      if (serialCommands.length === 1) {
        ...
        dispatchCommand(command, args, repos, envConfig);
      } else {
        // 串行执行多条指令
        const results = [];
